...
class Attention(Layer):
    def __init__(self, hidden, init='glorot_uniform', activation='linear', W_regularizer=None,
                 W0_regularizer=None, W_constraint=None, W0_constraint=None, **kwargs):
...
class myFlatten(Layer):
    ...
    def compute_output_shape(self, input_shape):
        return (input_shape[0], np.prod(intput_shape[1:]))
    ...
    def get_config(self):
        ...
        base_config = super(myFlatten, self).get_congif()
        return dict(list(base_config.items()) + list(config.items()))
